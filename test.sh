...
docker cp $CONTAINER_NAME:$TARGET_DIR/usr/share/postgresql/13/extension/ $HOST_DIR/
docker cp $CONTAINER_NAME:$TARGET_DIR/usr/lib/postgresql/13/lib/hello_pg_ext.so $HOST_DIR/lib/$EXTENSION_NAME.so
...
docker cp $HOST_DIR/lib/$EXTENSION_NAME.so $POSTGRES_CONTAINER_NAME:/usr/lib/postgresql/13/lib/$EXTENSION_NAME.so

# Copy the extension files from the host to the PostgreSQL container
docker cp $HOST_DIR/extension $POSTGRES_CONTAINER_NAME:/usr/share/postgresql/13/
